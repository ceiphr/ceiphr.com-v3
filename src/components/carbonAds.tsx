...
class carbonAds extends Component<{}, State> {
    ...
    componentDidMount() {
        const carbon_wrapper = document.querySelector(`.${this.state.carbonWrapper ? this.state.carbonWrapper : 'carbon-ads-wrapper'}`);
    ...
    render() {
        return (
            <div className={`carbon-ads-wrapper ${this.state.customClass}`}></div>
        );
    }
}
